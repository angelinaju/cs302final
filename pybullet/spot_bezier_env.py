...
class spotBezierEnv(spotGymEnv):
  ...
  """
    ...
    """
        ...
        # Get ball posistion 
        self.sphere_target_pos, _ = self._pybullet_client.getBasePositionAndOrientation(self.sphere_id)

        # Calc dist to ball and create target positions, I think this is where I may be messing up?
        robot_pos = np.array(self._last_base_position)
        vec_to_target = np.array(self.sphere_target_pos[:2]) - np.array(robot_pos[:2])
        target_yaw = np.arctan2(vec_to_target[1], vec_to_target[0])
        target_distance = np.linalg.norm(vec_to_target)
    ...
    def _reward(self):
        ...
        # Get posisitions and create a reward based on distance
        robot_pos = np.array(self.spot.GetBasePosition())
        self.sphere_target_pos, _ = self._pybullet_client.getBasePositionAndOrientation(self.sphere_id)
        dist_to_target = np.linalg.norm(robot_pos[:2] - self.sphere_target_pos[:2])
        ball_reward = dist_to_target
        print(ball_reward)
        ...
        reward = (self._distance_weight * forward_reward +
                  ...
                  self._ball_weight * ball_reward) 
        self._objectives.append(
            [forward_reward, energy_reward, drift_reward, shake_reward, ball_reward])
        # print("REWARD: ", reward)
