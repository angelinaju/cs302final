...
class spotGymEnv(gym.Env):
  ...
  """
    ...
    def _reward(self):
        ...
        # calculating angle between ball and robot
        ball_pos = pybullet.getBasePositionAndOrientation(self.sphere_id)[0]
        robot_pos = self.spot.GetBasePosition()
        # angle in radians
        angle = math.atan2(ball_pos[1] - robot_pos[1],
                           ball_pos[0] - robot_pos[0])
        facing_ball_reward = math.cos(angle)  # between -1 and 1

        distance_from_ball = math.sqrt(
            (ball_pos[0] - robot_pos[0])**2 + (ball_pos[1] - robot_pos[1])**2)
        distance_from_ball_reward = 1 - (distance_from_ball / 2.0)
        ...
        rot_reward = abs(obs[4])*facing_ball_reward
        ...
        reward = (self._distance_weight * distance_from_ball_reward*facing_ball_reward +
        ...
        self._objectives.append(
            [facing_ball_reward, distance_from_ball_reward, energy_reward, drift_reward, shake_reward])
        # print("REWARD: ", reward)
        return reward
