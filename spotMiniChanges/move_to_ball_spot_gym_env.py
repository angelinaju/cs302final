...
class spotGymEnv(gym.Env):
  ...
  """
    ...
    def set_ball_position(self):
        ...
        robot_position = self.spot.GetBasePosition()
        min_distance = 0.5
        
        # check if the ball is too close to the robot
        while True:
            x = np.random.uniform(-0.5, 0.5)
            y = np.random.uniform(-0.5, 0.5)
            z = 0.1
            ball_position = [robot_position[0] + x, robot_position[1] + y, z]
            distance = math.sqrt((ball_position[0] - robot_position[0])**2 +
                                (ball_position[1] - robot_position[1])**2)
    ...
    def create_ball(self):
        # Make ball
        visual_shape_id = pybullet.createVisualShape(shapeType=pybullet.GEOM_SPHERE,
                                        radius=0.1,
                                        rgbaColor=[1, 0, 0, 1])  # red
        collision_shape_id = pybullet.createCollisionShape(shapeType=pybullet.GEOM_SPHERE,
                                                    radius=0.1)
        # Set ball position
        ball_position = self.set_ball_position()

        self.sphere_id = pybullet.createMultiBody(baseMass=1,
    ...
    def reset(self,
        ...
        if self.sphere_id is not None:
            pybullet.removeBody(self.sphere_id)
        self.create_ball()
    ...
    """
        ...
        # get ball position and store it
        ball_position, _ = self._pybullet_client.getBasePositionAndOrientation(self.sphere_id)
        self.ball_position = ball_position

        # reward using current state
        reward = self._reward()
    ...
    def _reward(self):
        """ Reward includes:
        - Moving toward the ball (main goal)
        - Facing the ball
        - Stable walking (low roll/pitch, energy, drift)
        """
        ...
        roll, pitch, yaw = self._pybullet_client.getEulerFromQuaternion(orn)
        ...
        # POSITIVE FOR FORWARD, NEGATIVE FOR BACKWARD | NOTE: HIDDEN    
        fwd_speed = -Vb[3]  # body-frame forward velocity (vx)
        lat_speed = -Vb[4]  # body-frame lateral velocity (vy)
        ...
        robot_position = np.array(current_base_position)
        ball_position = np.array(self.ball_position)

        to_ball_vec = ball_position[:2] - robot_position[:2]
        to_ball_dist = np.linalg.norm(to_ball_vec)

        if to_ball_dist > 1e-5:
            to_ball_dir = to_ball_vec / to_ball_dist
        else:
            to_ball_dir = np.zeros(2)

        # robot direction from yaw
        robot_facing_dir = np.array([np.cos(yaw), np.sin(yaw)])

        # alignment reward, how well robot is facing the ball
        alignment = np.dot(robot_facing_dir, to_ball_dir)
        alignment_reward = alignment  # ranges [-1, 1]

        # approach reward, is it moving toward the ball
        lin_twist_world = np.array(self.spot.prev_lin_twist[:2])
        progress_toward_ball = np.dot(lin_twist_world, to_ball_dir)
        approach_reward = np.clip(progress_toward_ball, -1.0, 1.0)

        # combine into forward reward
        forward_reward = 0.7 * alignment_reward + 0.3 * approach_reward

        # rotation reward
        yaw_rate = obs[4]
        rot_reward = np.exp(-(yaw_rate - self.desired_rate)**2 / 0.1)
        ...
        energy_reward = -np.abs(
            np.dot(self.spot.GetMotorTorques(), self.spot.GetMotorVelocities())
        ) * self._time_step
        ...
        # Optional debug
        # print(f"Align: {alignment_reward:.2f}, Approach: {approach_reward:.2f}, Reward: {reward:.2f}")
    ...
    def set_time_step(self, control_step=0.05, simulation_step=0.001):
